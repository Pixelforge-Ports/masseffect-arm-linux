...
try {
    $buildArguments = @('build', '--progress=plain', '-t', 'masseffect-build', '-f', 'Dockerfile.build')
    ...
    & $dockerCommand.Source run --rm --mount "type=bind,source=$PSScriptRoot,target=/src" -w /src masseffect-build bash -lc 'make -j2 && make libs && bash package_portmaster.sh'
    if ($LASTEXITCODE -ne 0) { throw 'Port compilation or packaging failed; see the error above.' }
    Write-Host "Built: $PSScriptRoot/build/masseffect-portmaster.zip"
